...
class UserListAdapter(val context: Context, private val commonAlertDialog: CommonAlertDialog, private val isMultiSelection: Boolean,
    ...
    private fun setUserName(holder: ContactsViewHolder, profile: ProfileDetails) {
        if(searchKey.isNotBlank()) {
            val startIndex = profile.name!!.toLowerCase(Locale.getDefault()).indexOf(searchKey.toLowerCase(Locale.getDefault()))
            if (startIndex.isValidIndex()) {
                val stopIndex = startIndex + searchKey.length
                EmojiUtils.setEmojiTextAndHighLightSearchContact(context, holder.contactViewBinding.textChatName, profile.name, startIndex, stopIndex)
            } else
                holder.contactViewBinding.textChatName.text = profile.name
        } else {
            holder.contactViewBinding.textChatName.text = profile.name
        }
    }
