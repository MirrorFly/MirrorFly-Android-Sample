...
class BottomSheetOtpFragment(private val activity: Activity) : BottomSheetDialogFragment(),
        ...
        if (AppUtils.isNetConnected(activity)) {
            ...
            launch(exceptionHandler) {
                if (apiType == PinApi.REQUEST_OTP) {
                    ...
                    val mobileNUmber = Prefs.getString(SharedPreferenceManager.USER_MOBILE_NUMBER).replace(" ", "").replace("+", "")
    ...
    private fun resendVerificationCode() {
        ...
        val mobile = SharedPreferenceManager.instance.getString(SharedPreferenceManager.USER_MOBILE_NUMBER)
    ...
    override fun onResume() {
        super.onResume()
        setOtpTextViewEmpty(editTexts)
        if (isVisible && SharedPreferenceManager.instance.getBoolean(SharedPreferenceManager.CLOSE_DIALOG)) {
            SharedPreferenceManager.instance.storeBoolean(SharedPreferenceManager.CLOSE_DIALOG, false)
            dismiss()
        }
    }
