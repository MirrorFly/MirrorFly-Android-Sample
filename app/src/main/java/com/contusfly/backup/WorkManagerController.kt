...
object WorkManagerController {
    ...
    private fun getConstraint(isDownload: Boolean = false): Constraints {
        ...
        val isBackupOnlyOnWifi = Prefs.getBoolean(BackupConstants.WIFI_BACKUP_ONLY)
     ...
     */
    fun startBackingUp() {
        if (Prefs.getBoolean(BackupConstants.AUTO_BACKUP)) {
    ...
    private fun getDriveWithCleanWorker(wifiOnly: Boolean): Pair<OneTimeWorkRequest, OneTimeWorkRequest> {
        ...
        val driverWorker = driveUploadWorker.setInputData(
            Data.Builder().putBoolean(BackupConstants.IS_UPLOAD, true).putString(
                ...
                Prefs.getString(BackupConstants.BACKUP_FILE_PATH)
            ).build()
        )
            .build()
    ...
    fun resetBackupData() {
        Prefs.save(BackupConstants.BACKUP_INFO, emptyString())
        Prefs.save(BackupConstants.SHOULD_SHOW_RESTORE, false)
    }
    ...
    fun checkWifiLogic(): Boolean =
        Prefs.getBoolean(BackupConstants.WIFI_BACKUP_ONLY) && !isConnectedToWifi() && isNetConnected()
    ...
    fun backUpDriveUploadIsAutoBack(isBackupActivity: Boolean = false): UUID {
        cancelBackupWorkers()
        LogMessage.i(TAG, "AUTO_BACKUP ${Prefs.getBoolean(BackupConstants.AUTO_BACKUP)}")
        val scheduledTime = Prefs.getString(BackupConstants.BACKUP_FREQUENCY)
        val repeatedIntervels = when (scheduledTime) {
        ...
        val backUpWorker = PeriodicWorkRequest.Builder(
            BackUpDataWorker::class.java,
            repeatedIntervels,
            TimeUnit.DAYS
        ).setInputData(
            Data.Builder().putBoolean(BackupConstants.IS_PERIODIC, true)
                .putBoolean(BackupConstants.IS_AUTO_BACKUP, !isBackupActivity).build()
        )
            .addTag(BackupConstants.BACKUP_WORKER_TAG)
            .setConstraints(Constraints.Builder().setRequiresStorageNotLow(true).build())
            .build()
    ...
    fun backUpDriveUpload(isBackupActivity: Boolean = false): Triple<UUID, UUID, UUID> {
        ...
        val workers = getDriveWithCleanWorker(Prefs.getBoolean(BackupConstants.WIFI_BACKUP_ONLY))
