...
import com.contusfly.utils.AppConstants.UPLOAD_DONE
import com.contusflysdk.api.ChatManager
import com.contusflysdk.backup.*
...
class GoogleDriveWorker(private val appContext: Context, workerParams: WorkerParameters) :
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            ...
            if (drive == null) {
                ...
                if (isUpload) {
                    localBackupFilePath = Prefs.getString(BackupConstants.BACKUP_FILE_PATH)
                    try {
            ...
            exception?.let {
                ...
                when (it) {
                    ...
                    is UserRecoverableAuthIOException -> {
                        Prefs.save(BackupConstants.NEED_RELOGIN, true)
                        Prefs.save(BackupConstants.DRIVE_EMAIL, emptyString())
                        code = "401"
                        setProgress(
                            workDataOf(
                                BackupConstants.REASON to code,
                                BackupConstants.MESSAGE to appContext.getString(com.contusfly.R.string.drive_access_revoked),
                                BackupConstants.BACKUP_FILE_PATH to localBackupFilePath
                            )
                        )
                    }
    ...
    override fun progressChanged(downloader: MediaHttpDownloader?) {
        ...
        when (downloader!!.downloadState!!) {
            ...
            MediaHttpDownloader.DownloadState.MEDIA_COMPLETE -> {
                ...
                Prefs.save(BackupConstants.BACKUP_FILE_PATH, localBackupFilePath)
                launch(exceptionHandler) {
    ...
    private fun getBackUpFolderPath(): String {
        ...
        return rootFilePath + File.separator +
                ChatManager.applicationContext.resources.getString(com.contusfly.R.string.title_app_name)
    ...
    private fun getAppNameWithUserName() =
        ChatManager.applicationContext.getString(com.contusfly.R.string.title_app_name)
            .replace(" ", "") + "-" + Prefs.getString(
