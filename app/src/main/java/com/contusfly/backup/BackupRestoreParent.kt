...
open class BackupRestoreParent : BaseActivity(), CoroutineScope {
    ...
    fun showFrequencyDialog() {
        ...
        when (Prefs.getString(BackupConstants.BACKUP_FREQUENCY)) {
        ...
        dialogView.weeklyBox.setOnClickListener {
            setImageForImageView(1, listOf(dailyImage, weeklyImage, monthlyImage))
            Prefs.save(BackupConstants.BACKUP_FREQUENCY, BackupConstants.WEEKLY)
            Prefs.save(BackupConstants.NEXT_BACKUP_TIME, emptyString())
            dialog?.dismiss()
        }
        ...
        dialogView.monthlyBox.setOnClickListener {
            ...
            Prefs.save(BackupConstants.BACKUP_FREQUENCY, BackupConstants.MONTHLY)
            Prefs.save(BackupConstants.NEXT_BACKUP_TIME, emptyString())
            dialog?.dismiss()
        }
    ...
    fun checkAndLoginMail(accountName:String) {
        checkInternetAndExecute {
            val oldMail = Prefs.getString(BackupConstants.DRIVE_EMAIL)
            if (oldMail != "")
