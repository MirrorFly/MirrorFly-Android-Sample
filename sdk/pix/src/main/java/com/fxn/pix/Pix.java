...
import com.fxn.modals.MediaPreviewIntent;
...
public class Pix extends AppCompatActivity implements View.OnTouchListener {
    ...
    public void returnObjects() {
        ...
        mediaPreviewIntent.putExtra("file_path", list.get(0));
        mediaPreviewIntent.putExtra("from_camera", true);
        mediaPreviewIntent.putExtra("from_front_camera", options.isFrontfacing());
...
//        setResult(Activity.RESULT_OK, resultIntent);
    ...
    private void initialize() {
        ...
        camera.addCameraListener(new CameraListener() {
            ...
            public void onPictureTaken(PictureResult result) {
                ...
                result.toFile(photo, new FileCallback() {
                    ...
                    public void onFileReady(@Nullable File photo) {
                        String imgUrl = photo != null ? photo.getAbsolutePath() : "";
                        Utility.vibe(Pix.this, 50);
                        Img img = new Img("", "", imgUrl, "", 1);
                        selectionList.add(img);
            ...
            public void onVideoTaken(VideoResult result) {
                ...
                mediaMetadataRetriever.setDataSource(result.getFile().getPath());
                int duration = Integer.parseInt(mediaMetadataRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION))/1000;
                if (duration == options.getVideoDurationLimitinSeconds())
                    Toast.makeText(getBaseContext(), "Max limit for video recording is "+options.getVideoDurationLimitinSeconds() +" secs", Toast.LENGTH_LONG).show();
                returnObjects();
            }
    ...
    private void onClickMethods() {
        ...
        clickme.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                ...
                PermUtil.checkForAudioRecordPermissions(Pix.this, options, new WorkFinish() {
                    @Override
                    public void onWorkFinish(Boolean check) {
                        ...
                        if (flashDrawable == R.drawable.ic_flash_auto_black_24dp || flashDrawable == R.drawable.ic_flash_on_black_24dp)
    ...
    private void setFlashState(){
        if (flashDrawable == R.drawable.ic_flash_off_black_24dp) {
            camera.setFlash(Flash.OFF);
        } else if (flashDrawable == R.drawable.ic_flash_on_black_24dp) {
            camera.setFlash(Flash.ON);
        } else {
            camera.setFlash(Flash.AUTO);
        }
